...
exports.test = function() {
  ...
  var socket = io.connect('https://arenaws.topcoder.com', {transports: ['websocket', 'flashsocket', 'htmlfile', 'xhr-multipart', 'xhr-polling', 'jsonp-polling'], 'try multiple transports': true, reconnect: true, 'connect timeout': 5000, 'reconnection delay': 500});  

  socket.on('connect', function() {
    console.log('Client has connected to the server! Now logging in....');
    socket.emit('LoginRequest', {username: 'jeffdonthemic', password: process.env.PASSWORD});
  });
